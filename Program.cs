...
namespace ConsoleApplication4
{
    class ArrSort
    {
        public static IEnumerable<T> QSort<T>(IEnumerable<T> sortArray, Func<T, T, bool> res)
        {
            if (sortArray.Any())
            {
                ...
                var smltx = QSort(sortArray.Skip(1).Where(elem => res(elem,x)), res);
                var bgrtx = QSort(sortArray.Skip(1).Where(elem => res(x,elem)), res); 
                return smltx.Concat(new[] { x }).Concat(bgrtx);               
            }
    ...
    class Program
    {
        static void Main()
        {
            ...
            studentinfo = ArrSort.QSort(studentinfo, Student.StudentAVG).ToArray();
